...
impl Ecc384 {
    ...
    fn wait<F>(ecc: RegisterBlock<RealMmioMut>, condition: F) -> CaliptraResult<()>
    where
        F: Fn() -> bool,
    {
        let err_condition = || {
            ecc.intr_block_rf().error_global_intr_r().read().agg_sts()
                || ecc
                    .intr_block_rf()
        ...
        if err_condition() {
            ...
            ecc.intr_block_rf()
                .error_internal_intr_r()
                .write(|_| u32::from(ecc.intr_block_rf().error_internal_intr_r().read()).into());
